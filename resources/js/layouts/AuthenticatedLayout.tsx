...
export function AuthenticatedLayout({ children, navigation = [] }: { children: ReactNode; navigation?: NavigationItem[] }) {
    ...
    return (
        <div className="min-h-screen bg-canvas text-ink">
            <a href="#main-content" className="fixed left-4 top-4 z-tooltip -translate-y-24 rounded-lg bg-ink px-4 py-3 text-canvas focus:translate-y-0">
                {t('app.skipContent')}
            </a>
            <header className="sticky top-0 z-sticky border-b border-border bg-canvas/95">
                <div className="mx-auto flex min-h-16 max-w-7xl items-center gap-5 px-5">
                    <Link href="/" className="mr-auto rounded-md text-lg font-bold tracking-[-0.02em] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-focus">
                        {app.name}
                    </Link>
                    ...
                    <button
                        ...
                        className="flex min-h-11 items-center rounded-lg border border-border px-3 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus md:hidden"
