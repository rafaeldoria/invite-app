...
export function PublicLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-canvas text-ink">
            ...
            <header className="border-b border-border bg-canvas/95"><div className="mx-auto flex min-h-16 max-w-6xl flex-wrap items-center justify-between gap-x-4 gap-y-2 px-5 py-2"><Link href="/" className="rounded-md text-lg font-bold tracking-[-0.02em] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-focus">{app.name}</Link><Preferences locale={locale} setLocale={setLocale} isChanging={isChanging} t={t} /></div></header>
